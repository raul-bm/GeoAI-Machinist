...
public class ConvolutionalViewSolved : MonoBehaviour
{
    ...
    void Awake()
    {
        if(FindObjectsOfType<ConvolutionalViewSolved>().Length <= 1) viewCounter = 0;
    ...
    public void InitKernel(List<double> flatKernel, double[,] kernel)
    {
        ...
        kernelMatrix.transform.parent = kernelHolder.transform;
        kernelMatrix.transform.position = new Vector3(kernelHolder.transform.position.x + horizontalOffset, kernelHolder.transform.position.y + verticalOffset);
